...
void drawLineEx(float xi, float yi, float xf, float yf, float thick,
                const colorRGB& col, unsigned char alpha) {
  Color color = (Color){(unsigned char)col.r, (unsigned char)col.g,
                        (unsigned char)col.b, alpha};
  DrawLineEx((const Vector2){(float)xi, (float)yi},
             (const Vector2){(float)xf, (float)yf}, thick, color);
}
void drawLineBezier(float xi, float yi, float xf, float yf, float thick,
                    const colorRGB& col) {
  Color color = (Color){(unsigned char)col.r, (unsigned char)col.g,
                        (unsigned char)col.b, 255};
  DrawLineBezier((const Vector2){(float)xi, (float)yi},
                 (const Vector2){(float)xf, (float)yf}, thick, color);
}
...
void drawSymbol(int sym, int size, int x, int y, const colorRGB& col,
                unsigned char alpha, const string& font) {
  Color color =
...
void drawRing(const Vector2& center, float iRad, float oRad,
              const colorRGB& col, float alpha, float sAngle, float eAngle) {
  Color color = (Color){(unsigned char)col.r, (unsigned char)col.g,
                        (unsigned char)col.b, (unsigned char)alpha};
...
void drawGradientLineH(const Vector2& a, const Vector2& b, float thick,
  ...
  DrawRectangleGradientH(a.x, a.y - thick / 2.0, b.x - a.x, thick, colorB,
                         colorA);
}

void drawTextureEx(const Texture2D& tex, const Vector2& pos, float rot,
                   float scale) {
