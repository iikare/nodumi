...
class shaderData {
 public:
  ...
  void setShaderValue(const string& uf, const T& val, const int num = -1) {
    // verify types
    static_assert(
        is_same<T, Vector2>::value || is_same<T, Vector3>::value ||
            is_same<T, colorRGB>::value || is_same<T, float>::value ||
            is_same<T, int>::value || is_same<T, vector<Vector2>>::value ||
    ...
    constexpr int ufType =
        is_same<T, Vector2>::value || is_same<T, vector<Vector2>>::value
            ? SHADER_UNIFORM_VEC2
        : is_same<T, Vector3>::value || is_same<T, vector<Vector3>>::value
            ? SHADER_UNIFORM_VEC3
            :
            // cast to float
            is_same<T, colorRGB>::value || is_same<T, vector<colorRGB>>::value
            ? SHADER_UNIFORM_VEC3
        : is_same<T, float>::value || is_same<T, vector<float>>::value
            ? SHADER_UNIFORM_FLOAT
            :
            // 255 - unused value
            is_same<T, int>::value || is_same<T, vector<int>>::value
            ? SHADER_UNIFORM_INT
            : 255;
    ...
    if (ufType != it->second) {
      logW(LL_WARN, name, "- expecting uniform of type",
           to_string(it->second) + ",", "not", ufType);
      return;
    }
    ...
    if (isVecType) {
      ...
      if (num != -1) {
        logW(LL_WARN, name, "- invalid size parameter", num,
             "for non-variable uniform");
        return;
      }
    }
    ...
    if constexpr (isCRGB) {
      if constexpr (isVecType) {
        ...
        SetShaderValueV(shader, GetShaderLocation(shader, uf.c_str()),
                        vec_col.data(), it->second, num);
      }
      else {
        Vector3 conv = {static_cast<float>(val.r / 255.0f),
                        static_cast<float>(val.g / 255.0f),
                        static_cast<float>(val.b / 255.0f)};
        SetShaderValue(shader, GetShaderLocation(shader, uf.c_str()), &conv,
                       it->second);
      }
    }
    else {
      if constexpr (isVecType) {
        ...
        SetShaderValueV(shader, GetShaderLocation(shader, uf.c_str()),
                        val.data(), it->second, num);
      }
      else {
        SetShaderValue(shader, GetShaderLocation(shader, uf.c_str()), &val,
                       it->second);
      }
    }
  }
