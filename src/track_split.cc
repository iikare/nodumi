...
int findTrack(const note& n, const midi& m_stream, bool live, int numOn) {
  ...
  while (m_stream.notes[j].x >= 0 && j >= 0) {
    ...
    if (m_stream.notes[j].x + seqLimit <= n.x) {
      ...
      //logW(LL_WARN, "while BREAK on note:", m_stream.notes[j].x, m_stream.notes[j].y);

      break;
    }
    //logW(LL_WARN, "while on note:", m_stream.notes[j].x, m_stream.notes[j].y);
  ...
  for (auto considerNote : considerN) {
    //(m_stream.notes[considerNote].track ? tr1sum : tr0sum) += m_stream.notes[considerNote].y;
