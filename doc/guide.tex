...
\begin{tabular}{|l|l|}
  ...
  \mi{:if[w/h]}               													  & Fit Image Width/Height \\
  \mi{gg}   					 														        & Jump to Start \\
  \mi{G}   					 														          & Jump to End \\
...
\subsection{LSTM-Based Track Separation}
\label{subsec:lstm}

Using a small LSTM (dependent on \mi{libtorch}, available only on Linux, and requiring compile-time support), left/right hand detection from a single MIDI input can be done. This is primarily intended for use with instruments such as piano, organ, etc. If the program is compiled without \mi{libtorch} support, this feature will fall back to a heuristic-based algorithm for track separation, if enabled via the corresponding
\textcolor{lcol}{\hyperref[subsec:adaptivetrackdivision]{option}}.
...
\label{subsec:adaptivetrackdivision}
