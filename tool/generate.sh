...
echo "#define COMMIT_HASH \"$commit_hash\"" >> ./src/aghdef.h

if [ $# -gt 0 ]
then

  # tally files
  convert=("./data/font/" "./data/image/" "./data/model/")
  retype=("./data/shader/")
  ...
  do
    ...
    do
      if [[ ! -d "$file" ]]; then
        nfile=$(basename $file)
        ext_len=4
        if [[ $nfile =~ "input_class.pt" ]]
        then
          ext_len=3
        fi
        nfile=${nfile:0:-$ext_len}
        ...
        typedec=$(echo "${genfile:7:$ext_len}")
